...
export default function ProcessSection({
  ...
  const stepsToColumns = columns ?? steps.length;

  const gridCols =
    stepsToColumns === 3 ? 'lg:grid-cols-3 max-w-3xl' : 'lg:grid-cols-4';

  const isLite = variant === 'lite';

  return (
    <Section tone={tone} pattern={pattern}>
      <SectionHeader title={title} subtitle={subtitle} center={centerHeader} />

      <LazyMotion features={domAnimation}>
        <div className="relative pb-10 justify-items-center">
          <motion.ul
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, amount: 0.2 }}
            ...
            {steps.map((s, i) => {
              ...
              return (
                <motion.li
                  ...
                  whileHover={
                    prefersReduced || isLite
                      ? {}
                      : { rotateX: -3, rotateY: 3, z: 0 }
                  }
                  className="group relative"
                >
                  {/* Number badge (lite = plain, no gradient) */}
                  <span
                    aria-hidden
                    className={
                      isLite
                        ? 'absolute left-4 top-4 z-10 inline-flex h-9 w-9 items-center justify-center rounded-xl border border-foreground/10 bg-background text-sm font-semibold text-foreground/80'
                        : 'absolute left-[26px] top-6 z-10 bg-background shadow-[0_0_0_6px_rgba(0,0,0,0.03)] sm:left-[calc(50%-16px)] sm:top-auto sm:bottom-[-30px] inline-flex h-10 w-10 items-center justify-center rounded-xl border border-foreground/10 bg-[radial-gradient(60%_60%_at_50%_40%,#0047bb22,transparent),linear-gradient(180deg,#0047bb15,transparent)] text-sm font-semibold'
                    }
                  >
                    {i + 1}
                  </span>
                  ...
                  <Card
                    variant={isLite ? 'plain' : 'elevated'}
                    className={isLite ? '' : 'group-hover:shadow-lg'}
                  >
                    <div
                      className={
                        isLite
                          ? 'rounded-2xl border border-foreground/10 bg-background/60 p-6'
                          : 'rounded-2xl border min-h-45 sm:min-h-60 border-foreground/10 bg-background/70 p-6 backdrop-blur-sm'
                      }
                    >
                      <div className="mb-3 flex gap-3">
                        <h3 className="ml-16 mr-auto sm:ml-0 text-lg font-semibold leading-tight">
                          {s.title}
                        </h3>
                        ...
                        <span
                          className={
                            isLite
                              ? 'inline-flex h-9 w-9 items-center justify-center rounded-xl border border-foreground/10 bg-background'
                              : 'inline-flex h-10 w-10 items-center justify-center rounded-xl border border-foreground/10 bg-[radial-gradient(60%_60%_at_50%_40%,#0047bb22,transparent),linear-gradient(180deg,#0047bb15,transparent)] text-sm font-semibold'
                          }
                        >
                          <Icon className="h-5 w-5 text-primary" />
                        </span>
                      ...
                      {/* Fancy underline removed in lite */}
                      {!isLite && (
                        <div className="mt-4 h-px w-0 bg-gradient-to-r from-[var(--from,_#0047bb)] to-[var(--to,_#D33F49)] transition-all duration-300 group-hover:w-full" />
                      )}
                    </div>
