...
async function sendBrevoMail(
  ...
  if (!response.ok) {
    throw new Error(
      `[Brevo] ${failureLabel}: ${response.status} ${await response.text()}`,
    );
  }
}
...
export default async function sendEmail(payload: Record<string, any>) {
  ...
  <div style="font-family: Inter, Arial, sans-serif; max-width:640px; margin:auto; border:1px solid #e5e7eb; border-radius:16px; overflow:hidden">
    ...
    <div style="background:#0B0F1A; color:#fff; padding:16px; text-align:center">
      ...
      <p style="margin:4px 0 0; font-size:14px"><a href="${websiteUrl}" style="color:#0076FF; text-decoration:none">${websiteHost}</a></p>
    </div>
  </div>`;
  ...
  <div style="font-family: Inter, Arial, sans-serif; max-width:640px; margin:auto; border:1px solid #e5e7eb; border-radius:16px; overflow:hidden">
    ...
    <div style="padding:22px; color:#111827">
      ...
      <p style="text-align:center; margin:24px 0">
        <a href="${websiteUrl}" style="display:inline-block; padding:12px 18px; background:#0076FF; color:#fff; border-radius:10px; text-decoration:none; font-weight:700">Visit ALL8 Webworks</a>
      </p>
  ...
  const businessMail = {
    ...
    subject: `🔔 New ${payload.projectType} lead — ${payload.name}`,
  ...
  // The owner notification is the durable primary capture. Only acknowledge
  // the form after Brevo accepts it. Customer acknowledgement is secondary:
  // log its failure, but do not tell the user their captured lead was lost.
  await sendBrevoMail(businessMail, "Owner lead notification failed");

  try {
    await sendBrevoMail(customerMail, "Customer acknowledgement failed");
  } catch (error) {
    console.error("[Lead] Customer acknowledgement failed:", error);
  }
}
